...
cd "$SCRIPT_DIR"
RC=0
"$PYTHON" -m pytest smoke/ -v --timeout=120 -x --tb=short \
    --junitxml="$REPORT_DIR/smoke-junit.xml" \
    --html="$REPORT_DIR/smoke-report.html" --self-contained-html \
    "$@" || RC=$?

echo ""
echo "── Reports ──────────────────────────────────────────────────────"
echo "  HTML  : $REPORT_DIR/smoke-report.html"
echo "  JUnit : $REPORT_DIR/smoke-junit.xml"
exit $RC
